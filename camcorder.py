...
def load_preferences(filename=DEFAULT_PREFERENCES_PATH):
    try:
        ...
        print_red("Preferences file not found. Using default preferences.")
        preferences = default_preferences
    except json.JSONDecodeError:
        print_red("Corrupted or invalid preferences file. Using default preferences.")
        preferences = default_preferences
    return preferences
...
def validate_preferences(preferences):
    if not isinstance(preferences.get('destination_folder'), str):
        print_yellow(f"Invalid destination folder (must be a string). Using default preference: {default_preferences['destination_folder']}.")
    ...
    if not isinstance(preferences.get('in_secure_mode'), bool):
        print_yellow(f"Invalid in secure mode (must be true or false). Using default preference: {default_preferences['in_secure_mode']}.")
        preferences['in_secure_mode'] = default_preferences['in_secure_mode']
    if not isinstance(preferences.get('in_debug_mode'), bool):
        print_yellow(f"Invalid in debug mode (must be true or false). Using default preference: {default_preferences['in_debug_mode']}.")
        preferences['in_debug_mode'] = default_preferences['in_debug_mode']
    if not isinstance(preferences.get('root_camcorder'), str):
        print_yellow(f"Invalid root camcorder path (must be a string). Using default preference: {default_preferences['root_camcorder']}.")
...
def preferences_routine():
    ...
    if current_preferences["root_camcorder"] == "":
        print_red("The root camcorder path is not set in the preferences file.")
        print_red("Please set the root camcorder path in the preferences file or enter it now.")
        ...
        if current_preferences["in_secure_mode"]:
            ...
            if filecmp.cmp(source, destination_file_path, shallow=True): # check if the two files are identical based on metadata
                ...
                if hash_file == calculate_file_hash(destination_file_path):
                    print_green("Copy verification successful, the files are identical.")
                    has_been_copied = True
                else:
                    print_red("Error: The files content (checked with the ash) are not identical after copying.")
                    has_been_copied = False
            else:
                print_red("Error: The files metadata are not identical after copying.")
                has_been_copied = False

    except FileNotFoundError:
        print_red(f"Error: The file {source} does not exist.")
        has_been_copied = False
    except PermissionError:
        print_red("Error: Permission denied.")
        has_been_copied = False
    ...
    for video_path in video_files:
        ...
        with open(COPIED_FILES_LOG, 'r') as file:
            if hash_file in file.read():
                print_green(f"The file {os.path.basename(video_path)} has already been copied")
            else:
